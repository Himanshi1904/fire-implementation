...
parser.add_argument("--datapath_t1", type=str,
                    dest="datapath_t1", default='../Dataset',
                    help="data path for training images t1")
parser.add_argument("--datapath_t2", type=str,
                    dest="datapath_t2", default='../Dataset',
                    help="data path for training images t2")
...
datapath_t1 = opt.datapath_t1
datapath_t2 = opt.datapath_t2
...
def train_model():
    ...
    file_names_t1 = sorted(glob.glob(os.path.join(datapath_t1, "*.nii.gz")))
    file_names_t2 = sorted(glob.glob(os.path.join(datapath_t2, "*.nii.gz")))
    ...
    dataset = Dataset(file_names_t1, file_names_t2, iteration, True)
    ...
    for epoch in range(epochs):
        ...
        encoder_A.train(), encoder_B.train(), decoder_A_B.train(), decoder_B_A.train(), transformer_A_B.train(), \
            transformer_B_A.train(), spatial_transformer_deformable_A_B_32.train(), \
            spatial_transformer_deformable_B_A_32.train(), spatial_transformer_deformable_A_B_128.train(), \
            spatial_transformer_deformable_B_A_128.train()
        ...
        encoder_A.eval(), encoder_B.eval(), decoder_A_B.eval(), decoder_B_A.eval(), transformer_A_B.eval(), \
            transformer_B_A.eval(), spatial_transformer_deformable_A_B_32.eval(), \
            spatial_transformer_deformable_B_A_32.eval(), spatial_transformer_deformable_A_B_128.eval(), \
            spatial_transformer_deformable_B_A_128.eval()
        losses = []
        with torch.no_grad():
